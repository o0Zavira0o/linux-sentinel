...
- Status: **5F.3 FROZEN — corpus-v1 capture/audit/freeze complete; 5F.4 FROZEN — Structured Reasoner Output boundary CI-proven; 5F.5 FROZEN — Primary Metrics scorer and formal freeze transition CI-proven; 5F.6 ACTIVE — provider-neutral blinded execution-manifest/preflight implementation Fedora-validated, with checkpoint/CI freeze still pending; no concrete provider/model manifest, mandatory-ablation view, or scored reasoner output is frozen**
...
Phase-5F.6 now has a provider-neutral execution-preflight implementation that has passed authoritative Fedora validation. It adds one private function-only module and 16 dedicated tests; it imports neither hidden gold nor the frozen scorer and performs no provider call. It binds the frozen preregistration checkpoint, corpus archive, 5F.4 reasoner-output boundary, 5F.5 scorer, and 5E.4 FULL reference; fixes prompt-template SHA-256 `53924b77bb4b922fdebb484fa1022d56b782a902cb967034b1e0a4d8d5e2ab48` and output-schema SHA-256 `1874fe57f0efa0f03c65e9c0bfe2f022f824390d97e9a9a9e17bfdd463f4e788`; requires fresh stateless no-tool/no-web requests; pins provider/transport failure handling to at most one retry of the exact semantic request; and deterministically plans the 36 x 3 x 3 matrix only after all 108 full-request token counts prove adequate context capacity. Fedora validation passed 16/16 dedicated 5F.6 tests, 105/105 private Phase-5F tests, documentation integrity, and the full 1353/1353 unit-test gate with 158 Python files formatted, Ruff clean, and strict mypy clean across 85 source files. A concrete provider/model/version/configuration manifest instance is intentionally not selected by this generic boundary. Checkpoint commit, push, exact-SHA CI, and formal freeze of this implementation are still pending.
...
The candidate fixes the common task/prompt and output-schema digests, frozen-artifact identities, fresh stateless request shape, deterministic execution-order seed `sentinel-x.phase5f6-execution-order.v1`, and a pre-score transport policy of at most one retry of the exact request for provider/transport failure only. Invalid structured output is not a transport failure and remains unrepaired for the frozen scorer. The candidate accepts an explicit concrete provider/model/version/configuration/tokenizer manifest rather than choosing one.

Before the first score-bearing request, an operator must instantiate and freeze that concrete manifest against the frozen corpus, provide externally measured token counts for all 108 case-condition semantic requests, pass the exact 324-attempt no-truncation preflight, and freeze the corpus-specific mechanical construction of every mandatory field-ablation view. Those ablation identities remain execution/report metadata; the frozen M1–M8 scorer is not reopened to add new condition labels.

No scored benchmark result should be generated or inspected until those pre-score controls are frozen. The six preregistration files, frozen corpus-v1, frozen 5F.4 output boundary, frozen 5F.5 scorer, thresholds, and FULL Phase-5E.4 reference remain unchanged.
