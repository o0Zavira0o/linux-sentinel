...
- Current milestone: **5F.6 — Ablation; the provider-neutral blinded execution-manifest/preflight implementation is Fedora-validated but not yet checkpoint/CI-frozen. Authoritative validation passed 16/16 dedicated tests, 105/105 private Phase-5F tests, 1353/1353 full unit tests, 158-file format/lint scope, and strict mypy over 85 source files. No provider/model manifest instance, score-bearing request, or mandatory field-ablation view is frozen yet.**
...
- private Phase-5F.6 provider-neutral execution-preflight implementation that binds prompt/schema/frozen-artifact identities, stateless request construction, a one-retry transport policy, deterministic 324-attempt planning, and explicit no-truncation token-capacity checks; Fedora validation is complete, it performs no provider call, and checkpoint/CI freeze remains pending.
