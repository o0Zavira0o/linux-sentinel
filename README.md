...
- Current milestone: **5F.6 — Ablation; the provider-neutral blinded generic execution preflight is FROZEN at `39d1db2845f6d461ab21a6da2077be2b77bf5982`, with formal freeze commit `291fa32cea930b78d933c3dd35e11260c98bab3f` and exact-SHA CI run `32108809270`. The pre-score deterministic-baseline common-scoring adapter is Fedora-validated but not yet checkpoint/CI-frozen; it enables the frozen B1-vs-B1S HARD collapse rule to reuse the frozen 5F.5 metric primitives without reopening the scorer. Concrete provider/model/tokenizer controls, ablation mappings, and score-bearing requests remain blocked. No scored reasoner output exists.**
- Last authoritative implementation gate: **1365/1365 unit tests PASS**, **160 Python files** in the source/test format/lint scope, strict mypy over **86 source files PASS**; the deterministic-baseline common-scoring adapter also passed 12/12 dedicated, 27/27 frozen-scorer compatibility, and 117/117 private Phase-5F tests. Its checkpoint/CI freeze remains pending; no scored reasoner output exists
...
- private Phase-5F.6 deterministic-baseline scoring adapter, Fedora-validated but not yet checkpoint/CI-frozen, that preserves B0/B1/B1S identity outside the frozen scorer row while reusing the frozen scorer with each baseline's actual evidence projection;
