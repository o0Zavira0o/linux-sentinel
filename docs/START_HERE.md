...
- Current milestone: **5F.6 — Ablation; generic execution preflight and deterministic-baseline common-scoring adapter are FROZEN. The first local Ollama/Gemma execution-controls checkpoint `991fe365300ed6a507d0e961fda48ed4bad8e566` / CI `32155042040` is CI-PROVEN. Its 8192-context capacity assumption was then falsified pre-score by a preserved 108-request token map: 56/108 over budget, maximum 38091 tokens. Corrective context 49152 with the same f16/runtime/model/sampling controls is LIVE-PROVEN at the worst-case scale: frozen synthetic calibration 38087 tokens, exact tokenizer/inference equality, 14.208 generation tok/s, 79% GPU, peak 6735 MiB, zero swap. The active implementation task changes only context 8192→49152; max output 2048, safety 1024, model/GGUF identity, isolation and no-score-bearing-transport boundary remain unchanged. Corrected-manifest Fedora validation/checkpoint/CI, 108-count rebind, exact 324-attempt plan, mandatory-ablation mappings, deterministic scores, and LLM scores remain pending.**
...
- Update reason: the local Ollama/Gemma implementation checkpoint is now exact-SHA CI-proven, and subsequent pre-score evidence falsified only its 8192 context-capacity assumption. The frozen attempt-02 map completed 108/108 visible render/tokenizer counts with zero score-bearing generation and found 56 over-budget requests (maximum 38091). Corrective 49152 + f16 then passed a 38087-token live long-context proof with exact tokenizer/inference count equality, 79% GPU, 14.208 generation tok/s, and zero swap. The next engineering boundary is the repo-only 8192→49152 correction followed by Fedora gates/checkpoint/CI, then corrected-manifest 108-count rebind and exact 324-attempt no-truncation planning; benchmark scores remain absent.
