...
### 5F.6 — Ablation
Status: **ACTIVE — generic provider-neutral execution preflight and deterministic-baseline common-scoring adapter FROZEN; first local Ollama/Gemma execution-controls checkpoint CI-PROVEN; 8192 capacity FALSIFIED by the preserved 108-request token map; corrective 49152-context f16 candidate LIVE-PROVEN and undergoing repo correction/revalidation; no score-bearing request or benchmark score exists**.
...
The generic boundary leaves provider/model open. The deterministic B0/B1/B1S common-scoring adapter is frozen without reopening the scorer. Concrete Execution Freeze instantiates the provider slot locally as Ollama `0.32.13` with Gemma 4 12B IT QAT Q4_0, exact GGUF SHA `93567e57a8fe10b23569b9d9ec38cd005deedf71e29477c421a4b83f418a538b`, and Ollama model digest `be1d79d105352d8cb0a25ee03f1f315935cc93fb4f0674422c2bb13be72fc025`. The service remains loopback-only, cloud-disabled, single-parallel, and single-loaded-model. The future score payload keeps thinking/truncation/context shifting off, no tools/web/format repair, temperature 1.0/top-p 0.95/top-k 64, three repeat seeds, f16 KV, and `keep_alive=0`; the module still provides no normal generation transport. The first local execution-controls implementation checkpoint `991fe365300ed6a507d0e961fda48ed4bad8e566` passed exact-SHA CI run `32155042040`. Its 8192-context configuration had passed a short synthetic resource proof, but the subsequently frozen 108-request visible-only render/tokenizer map validly falsified no-truncation capacity: 56/108 requests exceeded the 5120-token input budget (32 RAW, 24 FULL, 0 MINIMAL), maximum `CASE-0028:raw = 38091`; map SHA `a2ddadf4fb93ad66589277122cdc40091f61ca488ba0f0be5f3f3de03f649ab2`. No hidden stream or score-bearing generation was used. Corrective context 49152 was therefore selected before any benchmark score while max output 2048 and safety 1024 remained unchanged, yielding a 46080-token input budget. Short f16 proof was CONDITIONAL PASS at 79% GPU/zero swap. A separate synthetic calibration froze 38087 prompt tokens, and live inference reproduced that count exactly, generated at 14.208 tok/s, retained 79% GPU placement with peak 6735 MiB, and used zero swap. This justifies the repo-only 8192→49152 correction; the corrected source deterministically yields manifest candidate SHA-256 `e0da42efc5e67fc3d86b434a99e3e46dda257ba1a8b049c2adbb408e79582ee6` but does not itself freeze that manifest. Fedora validation/checkpoint/CI, a 108-count rebind under the new manifest identity, and the exact 324-attempt generic no-truncation plan remain mandatory before Concrete Execution Freeze. Mandatory field-ablation views remain unconstructed, and all scored inference remains blocked.
