...
### 5F.6 — Ablation
Status: **ACTIVE — generic provider-neutral execution preflight FROZEN; deterministic-baseline common-scoring adapter FROZEN at `6d86fff7d0699ca4d11d48cdd98538ed81eb0380` with exact-SHA CI run `32123764282`; concrete provider/model/tokenizer execution controls, token-capacity map/plan, and mandatory-ablation mappings remain unfrozen pre-score work**.
...
The generic boundary deliberately does not choose a concrete provider/model and does not yet construct the mandatory field-ablation views. A subsequent pre-score audit identified a separate deterministic-scoring completeness issue: the frozen scorer condition field accepts only RAW/MINIMAL/FULL, while the preregistered B1-vs-B1S HARD collapse rule requires the same metric semantics for deterministic outputs. The frozen corrective adapter does not reopen the scorer. It carries `B0`/`B1`/`B1S` identity outside the frozen scorer row, uses each baseline's actual evidence condition (`minimal`, `minimal`, `full`), uses repeat index 1 only as a scorer-transport sentinel, requires deterministic M8 to remain unavailable, and validates the exact 108-output matrix before exposing HARD B1/B1S comparison summaries. Authoritative Fedora validation, exact stage/repository guards, checkpoint `6d86fff7d0699ca4d11d48cdd98538ed81eb0380`, push/local-remote equality, clean-repository verification, and exact-SHA four-job CI run `32123764282` all passed before any score-bearing output existed. The concrete manifest, tokenizer-measured 108-request capacity map, resulting 324-attempt execution plan, and exact corpus-specific mechanical construction of every mandatory ablation must still be frozen. Ablation identity remains execution/report metadata; the frozen M1–M8 scorer is not reopened.
