...
Phase-5F.3A adds private corpus/lineage/export contracts, empirical binding, five deterministic adversarial transformations, and an operator-only research capture harness. The initial 5F.3B attempt exposed a post-fault sampling race in that harness, so the corrective 5F.3A gate adds dedicated tests for the bounded post-fault sample condition: a sample exactly at/after fault end is accepted, a stream ending before fault end is rejected, and sidecar errors take precedence. The expected corrective baseline adds three unit tests to the prior 1303-test suite. Because `scripts/check.sh` does not include `research/`, `research/phase5f/capture_corpus_v1.py` still requires separate compile, Ruff, and direct mypy validation before corrective freeze. Live corpus capture resumes only after the corrective commit/CI is frozen.
