...
- Active milestone: **5F.6 — Ablation; the private provider-neutral execution-manifest/preflight implementation is Fedora-validated but not yet checkpoint/CI-frozen. Authoritative validation passed 16/16 dedicated tests, 105/105 private Phase-5F tests, 1353/1353 full unit tests, 158-file format/lint scope, and strict mypy over 85 source files. It pins prompt/schema identity, exact model/version/config manifest fields, one exact-request transport retry, stateless no-tool/no-web requests, deterministic 324-attempt ordering, and externally measured token-capacity checks. A concrete provider/model manifest instance and exact mandatory-ablation views are not yet frozen; no score-bearing request is authorized.**
