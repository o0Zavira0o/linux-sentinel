...
1. complete this documentation-only formal Mandatory Ablation Mapping Freeze transition and require green exact-SHA CI before treating the repository freeze as authoritative;
2. freeze the exact score-bearing execution semantics for all mandatory ablation views without changing the already frozen field mappings;
3. keep `scored_execution_authorized=False` until that execution boundary is content-addressed and independently audited;
