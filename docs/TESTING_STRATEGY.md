...
The local Phase-5F.6 Ollama/Gemma execution-controls candidate adds one private provider adapter plus **18 dedicated tests**. The tests pin exact Ollama/model/GGUF identity, loopback/no-cloud single-request service assumptions, renderer/parser/template identity, candidate context/output/safety budgets, three distinct repeat seeds, Gemma sampling controls, no tools/web/thinking/truncation/context shifting/schema repair, pure future score-payload construction, debug-render-only parsing, llama-server tokenizer semantics, canonical payload digests, bounded loopback HTTP, runtime identity verification, one-attempt failure surfacing, and the architectural absence of normal score-bearing `/api/chat` transport. The first live identity proof revealed that Python 3.14 `http.client.HTTPConnection.request()` rejects `headers=None` for the adapter's bodyless GET calls; the source now passes `{}` for GET and the existing runtime-identity unit test asserts empty GET headers and JSON POST headers. Authoritative revalidation passed **18/18 dedicated tests**, **34/34 generic-execution + local-adapter tests**, **135/135 private Phase-5F tests**, **1383/1383 total unit tests**, **162 Python files** in format/lint scope, and strict mypy success across **87 source files**. Live non-score-bearing validation then proved service-account access to the private model store without permission broadening, exact debug-render/`/tokenize` equivalence at **76/76 tokens** from a 401-character rendered prompt, complete tokenizer-process cleanup, and a context-8192 Strong resource PASS at **87% GPU / 13% CPU** with zero swap. Frozen-corpus 108-count capture remains blocked until the implementation checkpoint is recorded and exact-SHA CI passes.
