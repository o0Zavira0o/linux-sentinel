...
Phase-5F.3A adds private corpus/lineage/export contracts, empirical binding, five deterministic adversarial transformations, and an operator-only research capture harness. The initial 5F.3B attempt exposed a post-fault sampling race; the first correction added three dedicated harness tests. A second live attempt then exposed a separate reverse-transform contract bug: an empirical `EFFECT_OBSERVED` case can legitimately lack `pairwise_evidence` and therefore lack a target `incident_timeline`. The second correction adds a live-shaped corpus regression proving reverse/counterevidence derivation synthesizes the adversarial target timeline from the observed anomaly rather than requiring a parent pairwise timeline. The expected full baseline becomes 1307 unit tests. `research/phase5f/capture_corpus_v1.py` still requires separate compile/Ruff/direct-mypy validation because `scripts/check.sh` does not cover `research/`.
