...
### 5F.6 — Ablation
Status: **ACTIVE — generic provider-neutral execution preflight and deterministic-baseline common-scoring adapter FROZEN; local Ollama/Gemma Concrete Execution candidate Fedora/LIVE-VALIDATED with implementation checkpoint pending; no score-bearing request or benchmark score exists**.
...
The generic boundary leaves provider/model open. The deterministic B0/B1/B1S common-scoring adapter is frozen without reopening the scorer. Concrete Execution Freeze now instantiates the provider slot locally as Ollama `0.32.13` with Gemma 4 12B IT QAT Q4_0, exact GGUF SHA `93567e57a8fe10b23569b9d9ec38cd005deedf71e29477c421a4b83f418a538b`, and Ollama model digest `be1d79d105352d8cb0a25ee03f1f315935cc93fb4f0674422c2bb13be72fc025`. The service is loopback-only, cloud-disabled, single-parallel, and single-loaded-model. The future score payload pins thinking/truncation/context shifting off, no tools/web/format repair, Gemma sampling temperature 1.0/top-p 0.95/top-k 64, three distinct repeat seeds, and `keep_alive=0`. The module provides no normal generation transport. Live identity validation discovered and corrected a Python 3.14 bodyless-GET header incompatibility, after which all dedicated/private/full regression gates re-passed. The private Ollama model-store permissions were preserved; tokenizer proof ran under the service account rather than broadening access. Ollama `_debug_render_only` plus loopback llama-server `/tokenize` exactly reproduced the known synthetic inference prompt count at 76/76 tokens and left no tokenizer process behind. A synthetic context-8192 run then passed the predeclared >=80% GPU resource threshold at 87% GPU / 13% CPU with zero swap. Candidate capacity remains 8192 context / 2048 output / 1024 safety until the exact 108 frozen-corpus token counts confirm or falsify no-truncation capacity. Mandatory field-ablation views remain unconstructed, and all scored inference remains blocked.
