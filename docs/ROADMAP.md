...
- Phase 5F: ACTIVE
- Milestone: 5F.6 — Ablation; the generic execution preflight is frozen, and the deterministic-baseline common-scoring adapter is frozen at `6d86fff7d0699ca4d11d48cdd98538ed81eb0380` with exact-SHA CI run `32123764282`. The adapter closes the pre-score baseline/scorer integration gap without reopening the frozen scorer. Concrete provider/model/tokenizer selection, 108-request token-capacity measurement, exact 324-attempt planning, mandatory ablation mapping, and all score inspection remain blocked; no scored reasoner output exists
