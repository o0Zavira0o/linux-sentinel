...
- Active milestone: **5F.6 — Ablation; the private provider-neutral generic execution preflight is FROZEN, and the deterministic-baseline common-scoring adapter is now FROZEN at implementation checkpoint `6d86fff7d0699ca4d11d48cdd98538ed81eb0380` after authoritative Fedora validation and exact-SHA four-job CI run `32123764282`. The adapter preserves the frozen scorer, metrics, thresholds, baselines, corpus, preregistration, and generic execution boundary unchanged. 5F.6 remains ACTIVE for the still-unfrozen concrete provider/model/tokenizer manifest, 108-request token-count map, exact 324-attempt plan, and mandatory-ablation mappings. No deterministic corpus score, provider call, score-bearing request, or scored reasoner output is authorized.**
