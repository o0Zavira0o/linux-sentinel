...
- Current milestone: **5F.6 — Ablation; generic execution preflight and deterministic-baseline common-scoring adapter are FROZEN. The local Ollama/Gemma execution-controls checkpoint `991fe365300ed6a507d0e961fda48ed4bad8e566` passed exact-SHA CI run `32155042040`, then its 8192-context capacity assumption was validly falsified before scoring by a frozen 108-request render/tokenizer map: 56/108 semantic requests exceeded the 5120-token input budget and the maximum request was 38091 tokens. The frozen map is preserved rather than retried or rewritten. Corrective live evidence supports context 49152 with the same f16 KV/runtime/model/sampling controls: a 38087-token synthetic prompt matched tokenizer and inference counts exactly, completed with 14.208 generation tok/s, 79% GPU placement, peak 6735 MiB GPU memory, and zero swap. The active repo correction changes only context 8192→49152; max output 2048, safety 1024, model/GGUF identity, sampling, isolation, and the absence of normal score-bearing transport remain unchanged. A corrected-manifest 108-count rebind and exact 324-attempt no-truncation plan are still required before formal Concrete Execution Freeze; no benchmark score exists.**
