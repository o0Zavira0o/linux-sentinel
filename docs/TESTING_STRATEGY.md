...
Phase-5F.6 execution-preflight implementation adds one private provider-neutral module plus **16 dedicated tests**. The tests pin manifest/frozen-artifact identity, prompt and output-schema digests, explicit model configuration and capacity, one exact-request provider/transport retry, fresh no-tool/no-web isolation, hidden-gold rejection, stateless request shape without condition/repeat disclosure, exact 36 x 3 x 3 planning, same-case outer-lineage equality, complete externally measured 108-request token-count coverage, no-truncation rejection, repeat semantic-request identity, deterministic execution order/plan hashing, and absence of gold/scoring/provider-runtime imports. The generic implementation performs no provider call and intentionally does not construct the mandatory corpus-specific field ablations. Authoritative Fedora validation confirms **16/16 dedicated 5F.6 tests**, **105/105 private Phase-5F tests**, **1353/1353 total unit tests**, **158 Python files** in format/lint scope, and strict mypy success across **85 source files**. Checkpoint commit/push/exact-SHA CI and formal freeze remain pending; no score-bearing request exists.
