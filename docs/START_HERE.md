...
1. complete this documentation-only formal Concrete Execution Freeze transition and require green exact-SHA CI before treating the repository freeze as authoritative;
2. freeze the exact corpus-specific mechanical view construction for every mandatory 5F.6 field ablation before inspecting any benchmark score;
3. prove the ablation mappings preserve case lineage, stable evidence references, missingness/counterevidence semantics, and the no-hidden-label boundary without provider or scoring calls;
4. only after the ablation freeze, execute B2 RAW, B3 MINIMAL, B4 FULL, and mandatory ablations with fresh isolated requests and logged exact-request transport retries only;
