...
- Current milestone: **5F.6 — Ablation; the generic execution preflight and deterministic-baseline common-scoring adapter are FROZEN. Concrete Execution Freeze is ACTIVE with local Ollama `0.32.13` / Gemma 4 12B IT QAT Q4_0 now Fedora/LIVE-VALIDATED but not formally frozen. Exact GGUF SHA `93567e57a8fe10b23569b9d9ec38cd005deedf71e29477c421a4b83f418a538b` and Ollama model digest `be1d79d105352d8cb0a25ee03f1f315935cc93fb4f0674422c2bb13be72fc025` are live-proven. The local service is loopback-only/no-cloud/single-request; direct user traversal of the Ollama model store remains denied while the `ollama` service account can read and hash the pinned blob. Debug-render plus tokenizer-only replay exactly reproduced the known synthetic prompt count (`76` tokens), and context 8192 passed at 87% GPU / 13% CPU with zero swap. The adapter cannot send a normal score-bearing chat request. Exact 108 frozen-corpus token counts, the concrete 324-attempt plan, mandatory ablations, and all benchmark scores remain absent.**
