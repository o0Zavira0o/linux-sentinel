...
- Current milestone: **5F.6 — Ablation; the provider-neutral blinded generic execution preflight is FROZEN at `39d1db2845f6d461ab21a6da2077be2b77bf5982` with exact-SHA CI run `32108226447`. The concrete provider/model/tokenizer manifest, 108-request token-count map, exact 324-attempt plan, and mandatory-ablation mappings remain unfrozen; no score-bearing reasoner output exists.**
...
- Update reason: 5F.5 is formally frozen after its CI-proven scorer checkpoint and separate docs-only freeze/5F.6-activation transition. The 5F.6 generic provider-neutral execution preflight is now also frozen at `39d1db2845f6d461ab21a6da2077be2b77bf5982` after authoritative Fedora validation and exact-SHA four-job CI run `32108226447`. The six frozen preregistration files, corpus-v1, 5F.4 output boundary, 5F.5 score semantics/thresholds, FULL reference, prompt/schema identities, and generic execution controls remain unchanged; no scored B2/B3/B4 output exists. Concrete provider/model/tokenizer execution controls, the complete 108-request token-count map, exact 324-attempt plan, and mandatory-ablation views remain blocked until separately frozen before the first score-bearing request.
