...
**5F.3A — Corpus Contract & Capture Harness: CORRECTIVE RE-FREEZE REQUIRED/ESTABLISHED BY THE CURRENT CORRECTIVE CHANGESET.** Private corpus modules keep the exact 36 opaque case IDs, 16-run empirical execution order, hidden `source_run_group`, same-case lineage audit, separated visible/hidden export, and five deterministic adversarial transformations. The first 5F.3B live attempt using `07e19d5dbb39f831a69e4b41b60624ec786558f8` failed before corpus export because the RAW sidecar was stopped immediately after the frozen runner returned and could miss the final controlled-fault boundary by less than one sampling interval. This was a harness correctness defect, not evidence disagreement: the existing validator correctly rejected the case. The corrective harness now waits, under an explicit timeout, for a completed sidecar round at or after the closed ground-truth fault end before stopping the sidecar. No reasoner was run, no score was computed, and the failed attempt is not corpus-v1.

**5F.3B — Live Corpus Capture & Freeze: PAUSED pending second 5F.3A correction.** The second attempt on `8638bc033d6010c0d98aec67802cb24f75def864` passed preflight and the corrected RAW boundary, then failed while deriving reverse/counterevidence because `_derive_reverse` required a parent target `incident_timeline`. Production empirical binding only emits that timeline when `pairwise_evidence` exists, while `EFFECT_OBSERVED` can also arise from controlled coverage alone. The transformation must therefore synthesize the adversarial target timeline from the already-observed target anomaly when pairwise timing is absent, mark its lineage as derived, and keep stale FULL records stripped. No corpus-v1 or score was produced. After corrective re-freeze, restart all 16 empirical executions from the beginning and then derive all 20 adversarial cases.
